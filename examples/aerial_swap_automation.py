    ...
    tx = pair_contract.execute(
        {
            "swap": {
                "offer_asset": {
                    "info": {"native_token": {"denom": "uc4e"}},
                    "amount": str(swap_amount),
                }
            }
        },
        sender=wallet,
        funds=str(swap_amount) + "uc4e",
    )
...
def _parse_commandline():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=1000000,
        help="initial uc4e balance to perform swaps using the liquidity pool",
    )
    parser.add_argument(
        "upper_bound",
        type=int,
...
def main():
    ...
    # tokens in trading wallet (currency will vary [uc4e,cw20] )
    currency = "uc4e"
    ...
    while True:
        ...
        if currency == "uc4e":
            # Calculate received cw20 tokens if uc4e tokens are given to LP
            tokens_out = round(
                ((cw20_amount * tokens) / (native_amount + tokens)) * (1 - commission)
            )

            # Sell price of uc4e => give uc4e, get cw20
            sell_price = tokens / tokens_out
            print("uc4e sell price: ", sell_price)
            ...
            # Calculate received uc4e tokens if cw20 tokens are given to LP
            tokens_out = round(
                ((native_amount * tokens) / (cw20_amount + tokens)) * (1 - commission)
            )

            # Buy price of uc4e => give cw20, get uc4e
            buy_price = tokens_out / tokens
            print("uc4e buy price: ", buy_price)
