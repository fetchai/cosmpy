...
from cosmpy.protos.cosmwasm.wasm.v1.query_pb2 import QueryCodesRequest, QuerySmartContractStateRequest
...
class LedgerContract:
    def __init__(self, path: str, client: LedgerClient, address: Optional[Address] = None):
        self._path = path
        self._client = client
    ...
    def store(self, key: PrivateKey, gas_limit: Optional[int] = None) -> int:
        ...
        gas_limit = gas_limit or 2000000  # TODO: Need to interface to the simulation engine
    ...
    def instantiate(self, code_id: int, args: Any, key: PrivateKey, label: Optional[str] = None,
                    gas_limit: Optional[int] = None, admin_address: Optional[Address] = None,
        ...
        tx = Transaction()
        tx.add_message(
            create_cosmwasm_instantiate_msg(code_id, args, label, sender_address, admin_address=admin_address,
                                            funds=funds))
        tx.seal(SigningCfg.direct(key, account.sequence), fee=fee, gas_limit=gas_limit)
        tx.sign(key, self._client.network_config.chain_id, account.number)
        tx.complete()
    ...
    def deploy(self, args: Any, key: PrivateKey, label: Optional[str] = None,
               store_gas_limit: Optional[int] = None, instantiate_gas_limit: Optional[int] = None,
        ...
        return self.instantiate(self._code_id, args, key, label=label, gas_limit=instantiate_gas_limit,
    ...
    def execute(self, args: Any, key: PrivateKey, gas_limit: Optional[int] = None,
        ...
        tx = Transaction()
        tx.add_message(
            create_cosmwasm_execute_msg(sender_address, self._address, args, funds=funds))
        tx.seal(SigningCfg.direct(key, account.sequence), fee=fee, gas_limit=gas_limit)
        tx.sign(key, self._client.network_config.chain_id, account.number)
        tx.complete()
