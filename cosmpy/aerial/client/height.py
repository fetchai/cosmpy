...
def _is_generated_grpc_stub(obj: Any) -> bool:
    """
    Heuristic (but deterministic for codegen): return True for instances of
    grpcio generated stub classes.

    Generated stubs live in modules named '*_pb2_grpc' and have class names like
    'QueryStub', 'ServiceStub', etc. There is no shared base class, so we use
    module identity.
    """
    t = type(obj)
    ...
    # Most reliable general discriminator for grpcio codegen
    if not mod.endswith("_pb2_grpc"):
        return False
...
class LedgerClientAtHeightView:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        1) Generated gRPC stubs (QueryStub, ServiceStub, ...)
        ...
        # 1) Wrap autogenerated gRPC stubs (no registry needed)
        if _is_generated_grpc_stub(obj):
